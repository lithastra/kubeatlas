...
```bash
cd ${KUBEATLAS_REPO:?set the repository checkout path}
...
cp ${KUBEATLAS_REPO:?set the repository checkout path}/plugins/atlas.yaml plugins/atlas.yaml
git add plugins/atlas.yaml
...
```bash
cd ${KUBEATLAS_REPO:?set the repository checkout path}/docs
npm install
npm run build
