...
For the v1.6.0 release-metadata-only promotion after the completed `1ad2ff4`
run, the release owner approved a separate one-hour functional check on
2026-09-24. See [the bounded exception](./v160-release-functional.md). It
does not change this runner, its 72-hour minimum, or its verifier. The old
evidence keeps its original commit and cannot be relabeled as a run of the
